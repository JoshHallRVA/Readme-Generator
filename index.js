...
async function getInfo() {
	...
	const userData = await inquirer.prompt([
		...
		},
		{
			type: "input",
			message:
				"What are the steps required to install your project? This could be an install command.",
			name: "install",
		},
		{
			type: "input",
		...
		},
		{
			type: "input",
			message:
				"Provide examples on how to run a test for your application. This could be a test command.",
			name: "test",
		},
		{
			type: "input",
			message:
				"Please state the names of all the contributors you would like to add. Seperate them with commas.",
			name: "contributors",
		},
		{
			type: "input",
			message:
				"Provide the github usernames of the contributors as well",
	...
	for (var i = 0; i < contributorGitName.length; i++) {
		...
		var contriInfo = await axios.get(
			`https://api.github.com/users/${contributorGitName[i]}`
		);
